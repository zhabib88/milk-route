...
const getCurrentMonthString = () => new Date().toISOString().slice(0, 7); // YYYY-MM
...
const CustomerForm = ({ onClose, onSave, defaultMonth, initialData = null }) => {
  ...
  const [scheduleMap, setScheduleMap] = useState(() => {
    ...
    const def = {}; DAYS_OF_WEEK.forEach(d => def[d] = 1); return def;
  });
...
export default function App() {
  ...
  const handleSaveCustomer = async (data) => {
    ...
    try {
      if (id) { await updateDoc(doc(db, 'artifacts', appId, 'public', 'data', `stores/${storeId}`, 'customers', id), { ...dataToSave, updatedAt: serverTimestamp() }); }
      else { await addDoc(collection(db, 'artifacts', appId, 'public', 'data', `stores/${storeId}`, 'customers'), { ...dataToSave, createdAt: serverTimestamp() }); }
    ...
    <div className="min-h-screen bg-gray-50 pb-20 font-sans text-slate-800">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 pb-safe z-20">
        <div className="max-w-md mx-auto flex justify-around items-center h-16">
          <button onClick={() => setActiveTab('today')} className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${activeTab === 'today' ? 'text-blue-600' : 'text-gray-400'}`}><Truck className="w-6 h-6" /><span className="text-[10px] font-bold">Today</span></button>
